// search + sort for the games grid. mirrors blog.js, but operates on cards in
// [data-game-grid] and reads pre-baked search text from each card's data-text.
...
if (grid) {
  const items = Array.from(grid.querySelectorAll('li[data-date]')).map(el => ({
    el,
    date: el.dataset.date || '',
    text: el.dataset.text || '',
  }));

  const order = () => localStorage.getItem('games-sort') || 'desc';
  ...
  const render = () => {
    ...
    visible.forEach(it => grid.appendChild(it.el));
  ...
  if (sortBtn) {
    sortBtn.addEventListener('click', () => {
      localStorage.setItem('games-sort', order() === 'asc' ? 'desc' : 'asc');
      render();
    });
  }
